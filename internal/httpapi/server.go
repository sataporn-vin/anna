...
type Application interface {
	...
	UpdateTransaction(context.Context, string, memory.TransactionUpdateInput) (bson.M, error)
...
func New(application Application, token string, maxBody int64, logger *slog.Logger) http.Handler {
	...
	mux.Handle("PATCH /v1/transactions/{id}", server.authenticate(http.HandlerFunc(server.updateTransaction)))
